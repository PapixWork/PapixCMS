	...
	public function dataview($query, $search=NULL)
	{
		...
		if($rowCount>0)
		{
			...
			<tr>
				...
				<td><img src="<?php print $site_url; ?>/themes/<?php print $theme_current_code; ?>/images/empire/<?php print $empire=get_player_empire(getAccountID($row['master'])); ?>.jpg" alt="<?php print emire_name($empire); ?>" title="<?php print emire_name($empire); ?>"></td>
	...
	{		
		...
		if($total_no_of_records > 0)
		{
			...
			for($i=$x;$i<=$x+3;$i++)
				...
				{
					if($search)
						print "<a class='current' href='".$self.$i."/".$search."' style='color:white;text-decoration:none'>".$i."</a>&nbsp;&nbsp;";
					else
						print "<a class='current' href='".$self.$i."' style='color:white;text-decoration:none'>".$i."</a>&nbsp;&nbsp;";
				}
